              ...
              <div className="flex-col flex flex-1 space-y-1.5">
                <Suspense fallback={<Skeleton className="h-12 w-3/4" />}>
                  <BlurFadeText
                    ...
                    yOffset={8}
                    text={`ehy! i'm ${DATA.name.split(" ")[0]} 👋`}
                  />
                </Suspense>
        ...
        <section id="skills">
          <div className="flex min-h-0 flex-col gap-y-3">
            <BlurFade delay={BLUR_FADE_DELAY * 9}>
              <h2 className="text-xl font-bold">Code Skills</h2>
            </BlurFade>
            <Suspense fallback={<Skeleton className="h-12 w-full" />}>
              <div className="flex flex-wrap gap-1">
                {DATA.skills.map((skill, id) => (
                  <BlurFade key={skill.name} delay={BLUR_FADE_DELAY * 10 + id * 0.05}>
                    <Badge key={skill.name} className="flex gap-1 px-2 ">
                      {skill.icon}
                      {skill.name}
                    </Badge>
                  </BlurFade>
        ...
        <section id="experience">
          <div className="flex min-h-0 flex-col gap-y-3">
            ...
            {DATA.experience.map((work, id) => (
              <Suspense key={work.company} fallback={<Skeleton className="h-32 w-full" />}>
                <BlurFade
                  key={work.company}
        ...
        <section id="education">
          <div className="flex min-h-0 flex-col gap-y-3">
            ...
            {DATA.education.map((education, id) => (
              <Suspense key={education.school} fallback={<Skeleton className="h-24 w-full" />}>
                <BlurFade
              ...
              <div className="flex flex-col items-center justify-center space-y-4 text-center">
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground md:text-xl/relaxed lg:text-base/relaxed xl:text-xl/relaxed">
                    I like code, and I like to build things. Here are some of the projects I&apos;ve worked on recently.
                  </p>
                </div>
              </div>
            </BlurFade>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 max-w-[800px] mx-auto">
              {DATA.projects.map((project, id) => (
                project.recentProject && (
                  <Suspense key={project.title} fallback={<Skeleton className="h-96 w-full" />}>
                    <BlurFade
                      key={project.title}
